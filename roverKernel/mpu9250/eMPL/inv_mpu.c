 ...
 *                  __MPU6050
 *                  __MPU6500
 *                  __MPU9150 (or __MPU6050 w/ AK8975 on the auxiliary bus)
 *                  __MPU9250 (or __MPU6500 w/ AK8963 on the auxiliary bus)
 */
...
#define i2c_write   HAL_MPU_WriteBytes
#define i2c_read    HAL_MPU_ReadBytes
#define delay_ms(x)    HAL_DelayUS(1000*x)
#define get_ms(x)      UNUSED(*x)
...
#if !defined __MPU6050 && !defined __MPU9150 && !defined __MPU6500 && !defined __MPU9250
#error  Which gyro are you using? Define __MPUxxxx in your compiler options.
#endif
 ...
 */
#if defined __MPU9150
#ifndef __MPU6050
#define __MPU6050
#endif                          /* #ifndef __MPU6050 */
#if defined AK8963_SECONDARY
#error "MPU9150 and AK8963_SECONDARY cannot both be defined."
#elif !defined AK8975_SECONDARY /* #if defined AK8963_SECONDARY */
#define AK8975_SECONDARY
#endif                          /* #if defined AK8963_SECONDARY */
#elif defined __MPU9250           /* #if defined__MPU9150 */
#ifndef __MPU6500
#define __MPU6500
#endif                          /* #ifndef__MPU6500 */
#if defined AK8975_SECONDARY
#error "MPU9250 and AK8975_SECONDARY cannot both be defined."
#elif !defined AK8963_SECONDARY /* #if defined AK8975_SECONDARY */
#define AK8963_SECONDARY
#endif                          /* #if defined AK8975_SECONDARY */
#endif                          /* #if defined__MPU9150 */
...
#ifdef __MPU6500
    /* __MPU6500 shares 4kB of memory between the DMP and the FIFO. Since the
 ...
 *  \n __MPU6050: 1.25Hz, 5Hz, 20Hz, 40Hz
 *  \n __MPU6500: 1.25Hz, 2.5Hz, 5Hz, 10Hz, 20Hz, 40Hz, 80Hz, 160Hz, 320Hz, 640Hz
 ...
 *  This function reads from the __MPU6500 accel offset cancellations registers.
 ...
 *  This function reads from the __MPU6050 accel offset cancellations registers.
 *  The format are G in +-8G format. The register is initialized with OTP 
 *  factory trim values.
 *  @param[in]  accel_bias  returned structure with the accel bias
 *  @return     0 if successful.
 ...
 *  The compass on the auxiliary I2C bus is read by the __MPU hardware at a
...
/**
 *  @brief      Read the __MPU interrupt status registers.
 ...
 *  Any __MPU register will clear the interrupt.
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if(otp_value_zero == 0) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if(debug) {
                log_i("Bias_Shift=%7d, Bias_Reg=%7d, Bias_HWST=%7d\r\n",
                        lroundf(st_shift_cust[i]), bias_regular[i],
                        bias_st[i]);
                log_i("OTP value: %7d\r\n", ct_shift_prod[i]);
            }
            ...
            if(debug)
                log_i("ratio=%7d, threshold=%7d\r\n", st_shift_ratio[i],
                            lroundf(test.max_accel_var));
        ...
        if(debug) {
            ...
            log_i("Min MG: %7d\r\n", lroundf(accel_st_al_min));
            log_i("Max MG: %7d\r\n", lroundf(accel_st_al_max));
        }
        ...
        for (i = 0; i < 3; i++) {
            ...
            if(debug)
                log_i("Bias_shift=%7d, st=%7d, reg=%7d\n", lroundf(st_shift_cust[i]), bias_st[i], bias_regular[i]);
    ...
    /* Self Test Pass/Fail Criteria C */
        ...
        if(debug)
            log_i("Accel:CRITERIA C: bias less than %7d\n", lroundf(accel_offset_max));
        for (i = 0; i < 3; i++) {
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if(otp_value_zero == 0) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if(debug)
                log_i("ratio=%7d, threshold=%7d\r\n", lroundf(st_shift_ratio[i]),
                            lroundf(test.max_gyro_var));
    ...
    /* Self Test Pass/Fail Criteria C */
        ...
        if(debug)
            log_i("Gyro:CRITERIA C: bias less than %7d\n", lroundf(gyro_offset_max));
        for (i = 0; i < 3; i++) {
...
int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    if(debug) {
        log_i("Accel offset data HWST bit=%d: %7d %7d %7d\n", hw_test, accel[0], accel[1], accel[2]);
        log_i("Gyro offset data HWST bit=%d: %7d %7d %7d\n\n", hw_test, gyro[0], gyro[1], gyro[2]);
    }
...
/**
 *  @brief      Trigger gyro/accel/compass self-test for __MPU6500/MPU9250
...
#define LOAD_CHUNK  (16)
    ...
    if (!firmware)
        return -1;
    //  It was noted that after each operation on DSP memory through mpu_*_mem(),
    //  a small delay is required in order for MPU to work properly
    //  TODO: Find better solution than delays
    for (ii = 0; ii < length; ii += this_write) {
        this_write = min(LOAD_CHUNK, length - ii);

        //  Read current data in memory block
        mpu_read_mem(ii, this_write, cur);
        //HAL_DelayUS(100);
        //  If block already has firmware, skip uploading that block
        if (!memcmp(firmware+ii, cur, this_write))
            continue;
        //  Upload firmware chunk into memory block
        ...
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
        //HAL_DelayUS(100);
        //  Verify that block of memory contains required firmware chunk
        if (memcmp(firmware+ii, cur, this_write))
            return -2;
    }
...
#ifdef __MPU9150
    /* For the __MPU9150, the auxiliary I2C bus needs to be set to VDD. */
 ...
 *  The behaviour of this feature is very different between the __MPU6050 and the
 *  __MPU6500. Each chip's version of this feature is explained below.
 *
 ...
 *
 *  \n __MPU6500:
 *  \n Unlike the __MPU6050 version, the hardware does not "lock in" a reference
 ...
 *  increments.
 *
 *  \n __MPU6500 Low-power accel mode supports the following frequencies:
 ...
 *  \n To select a fractional wake-up frequency, round down the value passed to
 *  @e lpa_freq.
 *  \n The __MPU6500 does not support a delay parameter. If this function is used
 *  for the __MPU6500, the value passed to @e time will be ignored.
 ...
 *  @}
 */
/*  CODE TO VERIFY DMP FIRMWARE
    for (ii = 0; ii < length; ii += this_write) {
        this_write = min(LOAD_CHUNK, length - ii);

        mpu_read_mem(ii, this_write, cur);
        UARTprintf("(%03d)Memory contains: \n", ii);
        for (jj=0; jj<LOAD_CHUNK;jj++)
            UARTprintf("0x%02X ", cur[jj]);
        UARTprintf("\n(%03d)Trying to write: \n", ii);
        //Verify memory before writing to it
        if (!memcmp(firmware+ii, cur, this_write))
            continue;
        ...
        if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii]))
            return -1;
        for (jj=0; jj<LOAD_CHUNK;jj++)
            UARTprintf("0x%02X ", firmware[jj+ii]);
        UARTprintf("\n(%03d)Memory now contains: \n", ii);
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
        for (jj=0; jj<LOAD_CHUNK;jj++)
            UARTprintf("0x%02X ", cur[jj]);
